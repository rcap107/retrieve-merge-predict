...
def prepare_input_data(df: pl.DataFrame, variable_of_interest):
    data = df.with_columns(
        (pl.col("source_table") +"|"+ pl.col("aggregation") +"|"+ pl.col("target_dl")).alias("key")
    ).select(
        pl.col("key"),
        pl.col(variable_of_interest).alias("var")
    ).to_pandas()
...
def prepare_boxplot(df, variable_of_interest, ylabel, yscale="lin"):
    ...
    formatted_data, labels = prepare_input_data(df, variable_of_interest=variable_of_interest)
    ...
    bp = plt.boxplot(formatted_data, notch=False, sym='+', vert=True, whis=1.5, widths=1)
    ...
    ax2.xaxis.set_minor_formatter(ticker.FixedFormatter(['Company Employees', 'Movies', "US Presidential Elections"]))
